    ...
    <div style={pageBg}>
      ...
      <div style={{maxWidth:"900px",margin:"0 auto",padding:"30px 20px",position:"relative",zIndex:1}}>
        <div style={cardStyle}>
          ...
          {/* Header — espresso-steel gradient matching all other forms */}
          <div style={{background:"linear-gradient(160deg, #3E3830 0%, #4E4840 50%, #3E3830 100%)",borderBottom:"1px solid rgba(212,160,23,0.3)",padding:step===1?"50px 30px":"30px",textAlign:"center",position:"relative",overflow:"hidden"}}>
            {/* SVG rings */}
            ...
            <div style={{position:"relative",zIndex:1}}>
              <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"8px"}}>
                <div style={{fontFamily:"'Oswald', sans-serif",fontSize:step===1?"52px":"42px",fontWeight:"300",letterSpacing:"8px",color:"white",textShadow:"0 0 20px rgba(255,255,255,0.6), 0 0 40px rgba(240,192,64,0.3)"}}>CLEANING</div>
                <div style={{fontFamily:"'Allura', cursive",fontSize:step===1?"56px":"46px",letterSpacing:"3px",marginTop:"-8px",fontWeight:"400",background:"linear-gradient(180deg, #FFF0A0 0%, #F0C040 25%, #C8900A 50%, #F0C040 75%, #FFF0A0 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text"}}>Su Oficina</div>
              </div>
              {step===1&&(
                <>
            ...
            {step===1&&(
              <div className="fade-in-up">
                <div style={{textAlign:"center",marginBottom:"50px"}}>
                  <div style={{width:"110px",height:"110px",background:"linear-gradient(135deg,#A07B15,#D4A017,#F0C040)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 30px",boxShadow:"0 20px 50px rgba(212,160,23,0.25)",border:"4px solid rgba(255,255,255,0.9)"}}>
                    <div style={{fontSize:"55px"}}>💼</div>
                  </div>
                  <h2 style={{fontSize:"36px",fontWeight:"900",color:"#4A3728",margin:"0 0 20px",letterSpacing:"-1px",textTransform:"uppercase"}}>Let's Get Started!</h2>
                  <p style={{color:"#666",fontSize:"18px",lineHeight:"1.6",fontWeight:"500"}}>Professional cleaning for your business</p>
                </div>
                ...
                <div style={{maxWidth:"500px",margin:"0 auto"}}>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px",marginBottom:"25px"}}>
                    {[["First Name","John",firstName,setFirstName],["Last Name","Smith",lastName,setLastName]].map(([lbl,ph,val,set])=>(
                      <div key={lbl}>
                        <label style={{display:"block",fontSize:"14px",fontWeight:"800",color:"#A07B15",marginBottom:"12px",letterSpacing:"1.5px",textTransform:"uppercase"}}>{lbl}</label>
                        <input type="text" placeholder={ph} value={val} onChange={e=>set(e.target.value)} style={{...inputSt,fontSize:"17px",padding:"20px 24px"}}/>
                      </div>
                    ))}
                  </div>
                  {[["Email Address","email","john@company.com",email,setEmail],["Phone Number","tel","(907) 555-1234",phone,setPhone]].map(([lbl,type,ph,val,set])=>(
                    <div key={lbl} style={{marginBottom:"25px"}}>
                      <label style={{display:"block",fontSize:"14px",fontWeight:"800",color:"#A07B15",marginBottom:"12px",letterSpacing:"1.5px",textTransform:"uppercase"}}>{lbl}</label>
                      <input type={type} placeholder={ph} value={val} onChange={e=>set(e.target.value)} style={{...inputSt,fontSize:"17px",padding:"20px 24px"}}/>
                    </div>
                  ))}
                  <div style={{marginBottom:"40px"}}>
                    <label style={{display:"block",fontSize:"14px",fontWeight:"800",color:"#A07B15",marginBottom:"12px",letterSpacing:"1.5px",textTransform:"uppercase"}}>
                      Business Name <span style={{color:"#999",fontSize:"11px",textTransform:"none",fontWeight:"600"}}>(optional)</span>
                    </label>
                    <input type="text" placeholder="Acme Corp" value={businessName} onChange={e=>setBusinessName(e.target.value)} style={{...inputSt,fontSize:"17px",padding:"20px 24px"}}/>
                  </div>
              ...
              <div className="fade-in-up">
                ...
                <div style={{marginBottom:"35px"}}>
                  <label style={{display:"flex",alignItems:"center",fontSize:"14px",fontWeight:"800",color:"#A07B15",marginBottom:"15px",gap:"8px",letterSpacing:"1px",textTransform:"uppercase"}}>
                    <MapPin size={18} color="#A07B15"/> Business Address *
                  </label>
                  ...
                  <p style={{fontSize:"12px",color:"#A07B15",fontWeight:"600",marginTop:"4px"}}>📍 Start typing for address suggestions</p>
                ...
                <div style={{marginBottom:"35px"}}>
                  <label style={{display:"flex",alignItems:"center",fontSize:"14px",fontWeight:"800",color:"#A07B15",marginBottom:"15px",gap:"8px",letterSpacing:"1px",textTransform:"uppercase"}}>
                    <Building2 size={18} color="#A07B15"/> What type of business? *
                  </label>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"15px"}}>
                    {[
                      ...
                      <div key={cat.value} onClick={()=>setCategory(cat.value)} style={{padding:"25px 15px",borderRadius:"20px",cursor:"pointer",transition:"all 0.3s ease",textAlign:"center",
                        ...
                        <div style={{fontSize:"15px",fontWeight:"800",color:category===cat.value?"#0891B2":"#4A3728",marginBottom:"5px",letterSpacing:"0.5px"}}>{cat.label}</div>
                        <div style={{fontSize:"11px",color:"#888",fontWeight:"600"}}>{cat.desc}</div>
                      </div>
                    ))}
                  </div>
                </div>
