  ...
  const renderStep1 = () => (
    <div className="fade-in-up">
      ...
      <div style={{ display:"flex", gap:"15px" }}>
        ...
        <button onClick={goNext} disabled={!officeType} className="continue-btn" style={{ flex:2, padding:"18px", borderRadius:"16px", border: officeType?"2px solid rgba(200,144,10,0.5)":"2px solid rgba(212,160,23,0.2)", background: officeType?"linear-gradient(180deg, #FFF0A0 0%, #F8D860 20%, #D4A820 38%, #7A6535 50%, #D4A820 62%, #F8D860 80%, #FFF0A0 100%)":"rgba(255,255,255,0.4)", fontSize:"15px", fontWeight:"800", cursor:officeType?"pointer":"not-allowed", color:officeType?"#3D1800":"#B8A060", display:"flex", alignItems:"center", justifyContent:"center", gap:"8px", transition:"all 0.25s ease", boxShadow: officeType?"0 4px 14px rgba(200,144,10,0.35)":"none" }}>
  ...
  const renderStep2 = () => (
    <div className="fade-in-up">
        ...
        {[
          ...
          <div key={f.label} style={{ background:"linear-gradient(135deg,rgba(255,255,255,0.9),rgba(248,245,240,0.95))", border:"1px solid rgba(212,160,23,0.2)", borderRadius:"16px", padding:"16px", display:"flex", flexDirection:"column", gap:"10px" }}>
            ...
            <div style={{ display:"flex", gap:"8px" }}>
              ...
              <button onClick={()=>f.set(f.val+1)} style={{ flex:1, padding:"10px", borderRadius:"10px", border:"1.5px solid rgba(200,144,10,0.5)", background:"linear-gradient(180deg, #FFF0A0 0%, #F8D860 20%, #D4A820 38%, #7A6535 50%, #D4A820 62%, #F8D860 80%, #FFF0A0 100%)", color:"#3D1800", fontSize:"18px", fontWeight:"900", cursor:"pointer" }}>+</button>
            </div>
      ...
      <div style={{ display:"flex", gap:"15px" }}>
        ...
        <button onClick={goNext} disabled={!squareFeet||!frequency} className="continue-btn" style={{ flex:2, padding:"18px", borderRadius:"16px", border: (squareFeet&&frequency)?"2px solid rgba(200,144,10,0.5)":"2px solid rgba(212,160,23,0.2)", background: (squareFeet&&frequency)?"linear-gradient(180deg, #FFF0A0 0%, #F8D860 20%, #D4A820 38%, #7A6535 50%, #D4A820 62%, #F8D860 80%, #FFF0A0 100%)":"rgba(255,255,255,0.4)", fontSize:"15px", fontWeight:"800", cursor:(squareFeet&&frequency)?"pointer":"not-allowed", color:(squareFeet&&frequency)?"#3D1800":"#B8A060", display:"flex", alignItems:"center", justifyContent:"center", gap:"8px", transition:"all 0.25s ease", boxShadow: (squareFeet&&frequency)?"0 4px 14px rgba(200,144,10,0.35)":"none" }}>
          Continue <ChevronRight size={20}/>
        </button>
      </div>
    </div>
  ...
  return (
    <div style={BG_STYLE}>
      ...
      <style>{`
        ...
        .continue-btn:not(:disabled):hover { background: linear-gradient(180deg, #FFFAD0 0%, #FAE070 20%, #E0B830 38%, #8A7545 50%, #E0B830 62%, #FAE070 80%, #FFFAD0 100%) !important; color: #1A0800 !important; box-shadow: 0 6px 22px rgba(200,144,10,0.5) !important; border-color: rgba(200,144,10,0.7) !important; transform: translateY(-1px); }
        .continue-btn:not(:disabled):active { background: linear-gradient(180deg, #F0C040 0%, #D4A017 25%, #A07828 42%, #5A4828 50%, #A07828 58%, #D4A017 75%, #F0C040 100%) !important; color: #1A0800 !important; box-shadow: 0 2px 8px rgba(200,144,10,0.4) !important; transform: scale(0.98); }
